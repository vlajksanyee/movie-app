...
extension MediaItemDetail {
    func asMediaItem () -> MediaItem {
        ...
        return MediaItem(
            id: self.id, title: self.title, year: self.year, duration: String(self.runtime), imageUrl: self.imageUrl, rating: self.rating, voteCount: self.voteCount, mediaType: self.mediaType
        )
    }
}
